...
export default function ArticleForm({
	...
	const handleGenerateTitle = async () => {
		...
		try {
			const response = await geminiService.generateTitle(
				formData.body,
				controller.signal,
			);
			...
			setGenerationError(
				err instanceof Error
					? err.message
					: "Erreur lors de la génération du titre",
			);
		} finally {
	...
	const handleGenerateSubtitle = async () => {
		...
		try {
			...
			setGenerationError(
				err instanceof Error
					? err.message
					: "Erreur lors de la génération du sous-titre",
			);
		} finally {
			setGeneratingField(null);
		}
	...
	const handleGenerateSubhead = async () => {
		...
		try {
			const response = await geminiService.generateSubhead(
				formData.body,
				controller.signal,
			);
			...
			setGenerationError(
				err instanceof Error
					? err.message
					: "Erreur lors de la génération du chapeau",
			);
		} finally {
			setGeneratingField(null);
		}
	...
	const handleGenerateBody = async () => {
		...
		try {
			const response = await geminiService.generateBody(
				formData.body,
				controller.signal,
			);
			...
			setGenerationError(
				err instanceof Error
					? err.message
					: "Erreur lors de la correction du corps",
			);
		} finally {
			setGeneratingField(null);
		}
	...
	return (
		<>
			...
			{success && (
				<output className={styles.alert} data-type="success">
					...
					<p className={styles.formGroup}>
						<span className={styles.labelRow}>
							<label htmlFor="title" className={styles.label}>
								Titre{" "}
								<abbr title="requis" className={styles.required}>
									*
								</abbr>
							</label>
							<button
					...
					<p className={styles.formGroup}>
						...
						{!canGenerate && (
							<small className={styles.hintWarning}>
								Écrivez au moins {MIN_BODY_LENGTH} caractères pour activer la
								génération IA ({formData.body.trim().length}/{MIN_BODY_LENGTH})
							</small>
						)}
					</p>
				</fieldset>
